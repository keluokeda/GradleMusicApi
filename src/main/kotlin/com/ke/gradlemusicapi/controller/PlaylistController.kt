...
import com.ke.gradlemusicapi.entity.vo.RecentPlaylistVo
...
//        playlistRepository.saveAll(list)
     ...
     * 最近播放的歌单
     */
    @Operation(summary = "最近播放的歌单")
    @SecurityRequirement(name = "Bearer Authentication")
    @GetMapping("/playlist/recent")
    suspend fun recentPlaylists(authentication: Authentication): BaseVO<List<RecentPlaylistVo>> {
        val cookie = authentication.cookie
        val list = httpService.recentPlaylists(cookie).data.list
        return BaseVO.success(list.map {
            RecentPlaylistVo(
                playlistId = it.data.id,
                playTime = it.playTime,
                name = it.data.name,
                image = it.data.coverImgUrl
            )
        })
    }
