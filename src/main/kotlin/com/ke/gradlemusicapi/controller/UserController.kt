...
import com.ke.gradlemusicapi.entity.response.SameStatusUsersResponse
...
import com.ke.gradlemusicapi.entity.vo.UserStatusVO
...
class UserController(
    ...
    @Operation(summary = "当前用户状态")
    @SecurityRequirement(name = "Bearer Authentication")
    @GetMapping("/user/current/status")
    suspend fun userStatus(
        ...
        return BaseVO.success(
            UserStatusVO(
                current = httpService.userStatus(authentication.user.username, authentication.cookie).data.content,
                list = httpService.userSupportStatus(authentication.cookie).data
            )
        )
    ...
    @SecurityRequirement(name = "Bearer Authentication")
    @PutMapping("/user/current/status")
    suspend fun updateUserStatus(
        authentication: Authentication,
        content: String, type: String, iconUrl: String,actionUrl: String? = null
    ) : BaseVO<SameStatusUsersResponse.Data>{
        val cookie = authentication.cookie

        httpService.updateUserStatus(
            content,
            type,
            iconUrl,
            actionUrl,
        ...
        val response = httpService.sameStatusUsers(cookie)
